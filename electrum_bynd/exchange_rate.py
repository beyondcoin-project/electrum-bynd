...
class Bit2C(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bit2c.co.il', '/Exchanges/BYNDNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("BYND", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['BYND'][r]) for r in json['BYND']
                 if json['BYND'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=BYND")
        return json[ccy +'_BYND']


class Bitfinex(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitfinex.com', '/v1/pubticker/byndusd')
...
class Bitso(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.bitso.com', '/v3/ticker/?book=bynd_mxn')
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/bynd{ccy.lower()}/')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=BYND')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/beyondcoin/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/beyondcoin/history?interval=d1&limit=2000')
...
class CoinSpot(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['bynd']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        bynd_prices = json['prices']['BYND']
        return dict([(r, Decimal(bynd_prices[r])) for r in bynd_prices])
...
class HitBTC(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ccys = ['USD']
        json = await self.get_json('api.hitbtc.com', '/api/2/public/ticker/BYND%s' % ccy)
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XBYNDZ')]
        json = await self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XBYNDZ'+ccy]['c'][0])
        return result
    ...
    async def request_history(self, ccy):
        query = '/0/public/OHLC?pair=BYND%s&interval=1440' % ccy
        json = await self.get_json('api.kraken.com', query)
        history = json['result']['XBYNDZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=bynd_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('www.mercadobitcoin.net', '/api/bynd/ticker/')
...
class QuadrigaCX(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.quadrigacx.com', '/v2/ticker?book=bynd_cad')
