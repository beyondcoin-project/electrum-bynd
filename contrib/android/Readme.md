...
```
$ adb -d install -r bin/Electrum-BYND-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.electrum_bynd.electrum_bynd 1
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrum_bynd.electrum_bynd | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrum-bynd -g kivy`
...
```
$ adb shell
$ run-as org.electrum_bynd.electrum_bynd ls /data/data/org.electrum_bynd.electrum_bynd/files/data
$ run-as org.electrum_bynd.electrum_bynd cp /data/data/org.electrum_bynd.electrum_bynd/files/data/wallets/my_wallet /sdcard/some_path/my_wallet
```
